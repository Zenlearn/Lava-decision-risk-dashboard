export const DASHBOARD_DEFINITIONS = {
  ...
  orgKpiFootnote: 'Abbreviations & definitions for this page — BUSM: Business Unit Sales Manager (regional head). ASM: Area Sales Manager (territory supervisor). ASP: Authorised Service Partner (repair centre). S@H: Service at Home (doorstep repair). Universal Model Type Scope: Dashboard calculations & metrics apply strictly to "Smart" and "Element" models in the "Model type" column (Feature Phones and Tablets excluded). Rank Pill Color Legend: Top 20% = Green | 20%–50% = Blue | 50%–70% = Yellow | Below 70% = Red. CPC (₹): Cost Per Complaint = estimated leakage exposure ÷ flagged work orders. CPC Repair Cost Formula: Avg Repair Cost (₹) = ∑ (Total Part Value where Total Part Value > 0) ÷ Count of WOs (Total Part Value > 0). CPC Replacement Cost Formula: Avg Replacement Cost (₹) = ∑ (Handset Value where Call Type = Z9) ÷ Count of Z9 WOs. CAG: Composite Aggregate Grade — a weighted performance score across TAT, CPC, S@H adherence, NPS and Diagnostic Accuracy. TAT: Turnaround Time (work-order closure speed). DSAT: Detractor Satisfaction — customers who rated 1 or 2 ★ on the post-service survey. NPS rating categories: Promoters = Rating 5 ★ | Passives = Ratings 3–4 ★ | Detractors = Ratings 1–2 ★. All rupee values are provisional pending confirmation of the Lava part-cost master.',
